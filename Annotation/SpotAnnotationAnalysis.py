...
#			print(worker, "(", counter, "/", len(worker_list), ")")
	...
	def get_cluster_means(self, df, clustering_params):
		clusters = self.get_clusters(df, clustering_params)
		mean_coords = []
		for i in range(len(clusters.index)):
			row = clusters.iloc[[i]]
	...
	def plot_clusters(self, clusters, img_filename, img_filepath, img_height, csv_filepath, worker_marker_size, cluster_marker_size, correctness_threshold, bigger_window_size, plot_title):
		if bigger_window_size:
			fig = plt.figure(figsize=(14,12))
		else:
			fig = plt.figure(figsize = (12,7))
		...
		for i in range(len(clusters.index)):
			...
			# if i in self.get_clumped_list(clusters):
			# 	plt.scatter([centroid_x], [centroid_y], edgecolors = 'orange', s=50)

		# plot indiv annotations
		for i in range(len(member_lists)):			# for every cluster
			members = member_lists[i]					# get the list of annotations (w/ click properties) in that cluster
			if (cluster_correctness[i][1]):
				color = 'g'						
